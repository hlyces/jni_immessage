...
//#endif
	...
	*/
	...
	jbyteArray JNICALL Java_com_dffx_im_Security_IMBase64Eecode(JNIEnv* env, jobject obj,  jstring jstr)
	{
		...
		string strInData = std::string(pInData, nInLen);
		string strOutData = base64_encode(strInData);
		pTmp[32] = 0;
		env->ReleaseStringUTFChars(jstr, pInData);

		jbyteArray carr = env->NewByteArray(32);
		env->SetByteArrayRegion(carr, 0, strOutData.length(), (jbyte*)strOutData.c_str());
		free(pTmp);
		return carr;
	}
	...
	jbyteArray JNICALL Java_com_dffx_im_Security_IMBase64Decode(JNIEnv* env, jobject obj,  jstring jstr)
	{
		...
		string strInData = std::string(pInData, nInLen);
		string strOutData = base64_decode(strInData);
		pTmp[32] = 0;
		env->ReleaseStringUTFChars(jstr, pInData);
...
//        printf("%u\n", nOutLen);
	...
	string IMBase64Eecode(const string& pInData)
	{
		string strData = base64_encode(pInData);
		return strData;
	}
	
	string IMBase64Decode(const string& pInData)
	{
		string strData = base64_decode(pInData);
		return strData;
	}
